...
const articleID = ref("article-123.1");
const webPropertyName = "crosstalkDemo"; //our demo web property, you can make yours for free at https://crosstalk.cc/
const theme = computed(() => {
    if (props.isLight) return "light";
    else return "dark";
})
</script>
...
<template>
    <v-container fluid>
        <v-row>
            <v-col>
                <h1 class="ma-2">Imagine this is an article with some ID <code>{{ articleID }}</code></h1>
                <h3 class="ma-2">sample subtitle</h3>
            </v-col>
        </v-row>
        <v-row>
            <v-col>
        ...
        <v-row>
            <v-col>
                <CrossTalkComments class="ma-1" :pageID="articleID" :wp="webPropertyName" :theme="theme" />
            </v-col>
        </v-row>
    </v-container>
</template>